    ...
    private Movimentacao parseMovimentacao(String linha) {
        try {
            ...
            if (partes.length >= 7) {
                ...
                double distanciaPercorridaKm = partes.length > 7 && !partes[7].isBlank() ? Double.parseDouble(partes[7]) : 0.0;
                double litrosCombustivel = partes.length > 8 && !partes[8].isBlank() ? Double.parseDouble(partes[8]) : 0.0;

                return new Movimentacao(id, idVeiculo, idTipoDespesa, descricao, data, valor, tipo,
                        distanciaPercorridaKm, litrosCombustivel);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
